import React, { useEffect, useState, useContext } from 'react';
import { useSelector } from 'react-redux';
import toast, { Toaster } from 'react-hot-toast';
import { FaTrash, FaPlus, FaEye, FaRocket, FaTasks, FaProjectDiagram } from 'react-icons/fa';
import { ProjectSearchContext } from './Navbar';
import { Modal, Button } from 'react-bootstrap';
import 'bootstrap/dist/css/bootstrap.min.css';
import 'bootstrap/dist/js/bootstrap.bundle.min.js';
import './theme.css';

function BacklogPage() {
  const [projects, setProjects] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [allBacklogs, setAllBacklogs] = useState([]); // Store all backlogs from all projects
  const data = useSelector((state) => state.login.user);
  const theme = useSelector((state) => state.theme.mode);
  const { search, setSearch } = useContext(ProjectSearchContext);
  
  // Modal states
  const [showTeamModal, setShowTeamModal] = useState(false);
  const [teamLeads, setTeamLeads] = useState([{ name: '', email: '', password: '' }]);
  const [developers, setDevelopers] = useState([{ name: '', email: '', password: '', specialization: '', teamLeadIdx: 0 }]);
  const [showBacklogModal, setShowBacklogModal] = useState(false);
  const [backlogItems, setBacklogItems] = useState([]);
  const [newBacklog, setNewBacklog] = useState({ title: '', description: '' });
  const [showSprintModal, setShowSprintModal] = useState(false);
  const [sprintDesc, setSprintDesc] = useState('');
  const [selectedBacklog, setSelectedBacklog] = useState(null);
  const [showSprintsModal, setShowSprintsModal] = useState(false);
  const [sprints, setSprints] = useState([]);
  const [editSprintModal, setEditSprintModal] = useState(false);
  const [editSprint, setEditSprint] = useState(null);
  const [showTasksModal, setShowTasksModal] = useState(false);
  const [tasks, setTasks] = useState([]);
  const [selectedSprint, setSelectedSprint] = useState(null);
  const [newTask, setNewTask] = useState({ title: '', description: '', assigned_to: '' });
  const [selectedProjectId, setSelectedProjectId] = useState(null);
  const [selectedProjectForTasks, setSelectedProjectForTasks] = useState(null);
  // State variables for project selection modal
  const [showProjectSelectionModal, setShowProjectSelectionModal] = useState(false);
  // State variables for sprint functionality
  const [sprintEndDate, setSprintEndDate] = useState("");
  const [sprintEndDateError, setSprintEndDateError] = useState("");

  // Function to fetch all backlogs from all projects
  const fetchAllBacklogs = async (projectsList) => {
    const token = JSON.parse(sessionStorage.getItem('loggedInUser'))?.token;
    const backlogsWithProjects = [];
    
    for (const project of projectsList) {
      try {
        const res = await fetch(`http://localhost:3001/api/projects/${project.project_id || project.pid}/backlogs`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const backlogs = await res.json();
        
        if (Array.isArray(backlogs)) {
          backlogs.forEach(backlog => {
            backlogsWithProjects.push({
              ...backlog,
              projectName: project.pname || project.name,
              projectId: project.project_id || project.pid,
              project: project
            });
          });
        }
      } catch (err) {
        console.error(`Error fetching backlogs for project ${project.pname || project.name}:`, err);
      }
    }
    
    setAllBacklogs(backlogsWithProjects);
  };

  // Delete backlog function
  const handleDeleteBacklog = async (backlogId) => {
    if (!window.confirm("Are you sure you want to delete this backlog item?")) return;
    
    const token = JSON.parse(sessionStorage.getItem('loggedInUser'))?.token;
    try {
      const res = await fetch(`http://localhost:3001/api/backlogs/${backlogId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
      
      if (res.ok) {
        // Refresh all backlogs display
        await fetchAllBacklogs(projects);
        toast.success('Backlog item deleted successfully');
      }
    } catch (err) {
      console.error('Error deleting backlog:', err);
      toast.error('Failed to delete backlog item');
    }
  };

  // Open modal and prefill with current team if present
  const openTeamModal = (project) => {
    setShowTeamModal(true);
    setTeamLeads(
      Array.isArray(project.team_leads) && project.team_leads.length > 0
        ? project.team_leads.map(tl => ({ name: tl.name, email: tl.email, password: '' }))
        : [{ name: '', email: '', password: '' }]
    );
    setDevelopers(
      Array.isArray(project.developers) && project.developers.length > 0
        ? project.developers.map(d => ({ name: d.name, email: d.email, password: '', specialization: d.specialization || '', teamLeadIdx: 0 }))
        : [{ name: '', email: '', password: '', specialization: '', teamLeadIdx: 0 }]
    );
  };
  const closeTeamModal = () => setShowTeamModal(false);

  // Project selection modal functions
  const openProjectSelectionModal = () => {
    setShowProjectSelectionModal(true);
  };

  const closeProjectSelectionModal = () => {
    setShowProjectSelectionModal(false);
  };

  const handleProjectSelection = (project) => {
    closeProjectSelectionModal();
    openBacklogModal(project);
  };

  const handleTeamLeadChange = (idx, field, value) => {
    setTeamLeads(prev => prev.map((tl, i) => i === idx ? { ...tl, [field]: value } : tl));
  };
  const handleDevChange = (idx, field, value) => {
    setDevelopers(prev => prev.map((dev, i) => i === idx ? { ...dev, [field]: value } : dev));
  };
  const handleDevTeamLeadChange = (idx, value) => {
    setDevelopers(prev => prev.map((dev, i) => i === idx ? { ...dev, teamLeadIdx: value } : dev));
  };
  const addDeveloper = () => setDevelopers(prev => [...prev, { name: '', email: '', password: '', specialization: '', teamLeadIdx: 0 }]);
  const removeDeveloper = (idx) => setDevelopers(prev => prev.filter((_, i) => i !== idx));
  const addTeamLead = () => setTeamLeads(prev => [...prev, { name: '', email: '', password: '' }]);
  const removeTeamLead = (idx) => setTeamLeads(prev => prev.filter((_, i) => i !== idx));

  // Open backlog modal for a project
  const openBacklogModal = async (project) => {
    setShowBacklogModal(true);
    setSelectedBacklog(null);
    setNewBacklog({ title: '', description: '' });
    const token = JSON.parse(sessionStorage.getItem('loggedInUser'))?.token;
    try {
      const res = await fetch(`http://localhost:3001/api/projects/${project.project_id || project.pid}/backlogs`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
      setBacklogItems(Array.isArray(data) ? data : []);
      setSelectedProjectId(project.project_id || project.pid);
    } catch (err) {
      setBacklogItems([]);
    }
  };
  const closeBacklogModal = () => setShowBacklogModal(false);

  const handleAddBacklog = async () => {
    if (!newBacklog.title || !selectedProjectId) return;
    const token = JSON.parse(sessionStorage.getItem('loggedInUser'))?.token;
    try {
      const res = await fetch(`http://localhost:3001/api/projects/${selectedProjectId}/backlogs`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
        },
        body: JSON.stringify(newBacklog)
      });
      if (res.ok) {
        setNewBacklog({ title: '', description: '' });
        const updated = await fetch(`http://localhost:3001/api/projects/${selectedProjectId}/backlogs`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const updatedData = await updated.json();
        setBacklogItems(Array.isArray(updatedData) ? updatedData : []);
        
        // Refresh all backlogs display
        const projectsList = projects;
        await fetchAllBacklogs(projectsList);
        closeBacklogModal();
      }
    } catch (err) {
      setBacklogItems([]);
    }
  };



  const openSprintModal = (backlog) => {
    setSelectedBacklog(backlog);
    setSprintDesc('');
    setSprintEndDate('');
    setSprintEndDateError('');
    setShowSprintModal(true);
  };
  const closeSprintModal = () => setShowSprintModal(false);
  
  const handleAddToSprint = async () => {
    if (!selectedProjectId || !selectedBacklog) return;
    // Validate end date
    const today = new Date();
    const end = new Date(sprintEndDate);
    if (!sprintEndDate || isNaN(end.getTime()) || end <= today) {
      setSprintEndDateError("End date must be after today.");
      return;
    }
    setSprintEndDateError("");
    const token = JSON.parse(sessionStorage.getItem('loggedInUser'))?.token;
    try {
      const res = await fetch(`http://localhost:3001/api/projects/${selectedProjectId}/backlogs/${selectedBacklog.backlog_id || selectedBacklog.id}/add-to-sprint`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
        },
        body: JSON.stringify({ sprintDescription: sprintDesc, startDate: today.toISOString().slice(0,10), endDate: sprintEndDate })
      });
      if (res.ok) {
        const updated = await fetch(`http://localhost:3001/api/projects/${selectedProjectId}/backlogs`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const updatedData = await updated.json();
        setBacklogItems(Array.isArray(updatedData) ? updatedData : []);
        
        // Refresh all backlogs display
        const projectsList = projects;
        await fetchAllBacklogs(projectsList);
      }
    } catch (err) {
      setBacklogItems([]);
    }
    setShowSprintModal(false);
  };

  const openSprintsModal = async (project) => {
    setShowSprintsModal(true);
    const token = JSON.parse(sessionStorage.getItem('loggedInUser'))?.token;
    try {
      const res = await fetch(`http://localhost:3001/api/projects/${project.project_id || project.pid}/sprints`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
      setSprints(Array.isArray(data) ? data : []);
    } catch (err) {
      setSprints([]);
    }
  };
  const closeSprintsModal = () => setShowSprintsModal(false);
  const closeEditSprintModal = () => setEditSprintModal(false);

  const openTasksModal = async (sprint, project) => {
    setShowTasksModal(true);
    setSelectedSprint(sprint);
    setNewTask({ title: '', description: '', assigned_to: '' });
    const token = JSON.parse(sessionStorage.getItem('loggedInUser'))?.token;
    try {
      const res = await fetch(`http://localhost:3001/api/sprints/${sprint.sprint_id}/tasks`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
      setTasks(Array.isArray(data) ? data : []);
      setSelectedProjectForTasks(project);
    } catch (err) {
      setTasks([]);
    }
  };
  const closeTasksModal = () => setShowTasksModal(false);

  const handleAddTask = async () => {
    if (!newTask.title || !newTask.assigned_to || !selectedSprint) return;
    const token = JSON.parse(sessionStorage.getItem('loggedInUser'))?.token;
    try {
      const res = await fetch(`http://localhost:3001/api/sprints/${selectedSprint.sprint_id}/tasks`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
        },
        body: JSON.stringify(newTask)
      });
      if (res.ok) {
        setNewTask({ title: '', description: '', assigned_to: '' });
        const updated = await fetch(`http://localhost:3001/api/sprints/${selectedSprint.sprint_id}/tasks`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setTasks(await updated.json());
      }
    } catch (err) {}
  };

  const handleUpdateTaskStatus = async (taskId, status) => {
    const token = JSON.parse(sessionStorage.getItem('loggedInUser'))?.token;
    try {
      await fetch(`http://localhost:3001/api/tasks/${taskId}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
        },
        body: JSON.stringify({ status })
      });
      if (selectedSprint) {
        const updated = await fetch(`http://localhost:3001/api/sprints/${selectedSprint.sprint_id}/tasks`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setTasks(await updated.json());
      }
    } catch (err) {}
  };

  useEffect(() => {
    document.body.classList.toggle('dark', theme === 'dark');

    if (!data?.id) return;

    const token = JSON.parse(sessionStorage.getItem('loggedInUser'))?.token;
    setIsLoading(true);

    fetch(`http://localhost:3001/api/projects`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
      .then((res) => res.json())
      .then(async (projects) => {
        setProjects(projects);
        
        // Fetch all backlogs after getting projects
        await fetchAllBacklogs(projects);
        
        setIsLoading(false);
      })
      .catch((err) => {
        console.error('Error fetching projects:', err);
        setIsLoading(false);
      });

    return () => setSearch("");
  }, [data?.id, theme, setSearch]);

  const handleDelete = async (pid) => {
    const token = JSON.parse(sessionStorage.getItem('loggedInUser'))?.token;

    if (!window.confirm("Are you sure you want to delete this project?")) return;

    try {
      const response = await fetch(`http://localhost:3001/api/users/projects/${pid}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`
        }
      });

      const data = await response.json();

      if (!response.ok) {
        throw new Error(data.message || 'Failed to delete project');
      }

      setProjects(projects.filter(project => project.pid !== pid));
      alert('Project deleted successfully');
    } catch (err) {
      console.error('Error deleting project:', err);
      alert(err.message || 'Failed to delete project. Please try again.');
    }
  };

  // Filter projects based on role
  let displayProjects = projects;
  if (data?.role === 'project_manager') {
    displayProjects = projects.filter(
      p => p.created_by === data.id || p.created_by === data.user_id
    ).slice(0, 1);
  }

  // Filter backlogs based on search
  const filteredBacklogs = allBacklogs.filter((backlog) => {
    const searchLower = search.toLowerCase();
    return (
      (backlog.title && backlog.title.toLowerCase().includes(searchLower)) ||
      (backlog.description && backlog.description.toLowerCase().includes(searchLower)) ||
      (backlog.projectName && backlog.projectName.toLowerCase().includes(searchLower))
    );
  });

  // Group backlogs by project
  const backlogsByProject = filteredBacklogs.reduce((acc, backlog) => {
    if (!acc[backlog.projectId]) {
      acc[backlog.projectId] = {
        projectName: backlog.projectName,
        project: backlog.project,
        backlogs: []
      };
    }
    acc[backlog.projectId].backlogs.push(backlog);
    return acc;
  }, {});

  return (
    <div className="main-content-area">
      <div
        className="container-fluid w-100 user-projects-root"
        style={{
          background: theme === 'dark' 
            ? 'linear-gradient(135deg, #0f1419 0%, #1a1f2e 50%, #181c24 100%)' 
            : 'linear-gradient(135deg, #f8faff 0%, #e3f2fd 50%, #f4f6fb 100%)',
          color: theme === 'dark' ? '#f8f8f8' : '#181c24',
          transition: 'all 0.3s ease',
          paddingTop: '30px',
          minHeight: '91vh',
        }}
      >
        <div className="p-3">
          <div className="row justify-content-center g-3">
            <div className="col-12">
              <div
                className="shadow-lg rounded-3 p-3 mb-3 border-0"
                style={{
                  background: theme === 'dark' 
                    ? 'linear-gradient(145deg, #23272f 0%, #2b2f36 100%)' 
                    : 'linear-gradient(145deg, #ffffff 0%, #f8f9fa 100%)',
                  color: theme === 'dark' ? '#f8f8f8' : '#181c24',
                  boxShadow: theme === 'dark' 
                    ? '0 15px 30px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.1)' 
                    : '0 15px 30px rgba(0,0,0,0.08), inset 0 1px 0 rgba(255,255,255,0.8)',
                  backdropFilter: 'blur(10px)',
                  transition: 'all 0.3s ease'
                }}
              >
                <div className="text-center mb-3">
                  <h2 className="fw-bold mb-1" style={{
                    fontSize: '2rem',
                    color: theme === 'dark' ? '#f8f8f8' : '#181c24',
                  }}>
                    Project Backlogs
                  </h2>
                  <p className="text-muted mb-0" style={{ fontSize: '1.2rem' }}>Manage and organize your project backlogs efficiently</p>
                </div>

                {isLoading ? (
                  <div className="text-center p-4">
                    <div className="d-flex flex-column align-items-center">
                      <div className="spinner-border text-primary mb-2" style={{ width: '1.5rem', height: '1.5rem' }} role="status">
                        <span className="visually-hidden">Loading...</span>
                      </div>
                      <h6 className="text-muted" style={{ fontSize: '0.9rem' }}>Loading backlogs...</h6>
                    </div>
                  </div>
                ) : Object.keys(backlogsByProject).length === 0 ? (
                  <div className="text-center p-4">
                    <div className="mb-3">
                      <i className="fas fa-inbox" style={{ fontSize: '2rem', color: '#6c757d' }}></i>
                    </div>
                    <h5 className="text-muted mb-2" style={{ fontSize: '1.2rem' }}>No backlogs found</h5>
                    <p className="text-muted mb-3" style={{ fontSize: '1rem' }}>
                      Start by creating your first backlog item to organize your project tasks.
                    </p>
                    {(data?.role === 'project_manager' || data?.role === 'team_lead') && projects.length > 0 && (
                      <button
                        className="btn btn-primary btn-sm rounded-pill px-3 py-2"
                        onClick={openProjectSelectionModal}
                        style={{ 
                          fontWeight: '700',
                          fontSize: '0.9rem',
                          boxShadow: '0 4px 12px rgba(0,0,0,0.15)'
                        }}
                      >
                        <FaPlus className="me-1" />
                        Create First Backlog Item
                      </button>
                    )}
                  </div>
                ) : (
                  <div className="space-y-3">
                    {projects.map((project) => {
                      const projectId = project.project_id || project.pid;
                      const projectData = backlogsByProject[projectId] || { projectName: project.name || project.pname, backlogs: [], project };
                      return (
                        <div key={projectId} className="mb-4">
                          {/* Enhanced Project Header */}
                          <div className="project-header rounded-3 p-3 mb-3 border-0">
                            <div className="d-flex justify-content-between align-items-center">
                              <div className="d-flex align-items-center">
                                <div className="project-icon me-2" style={{ fontSize: '1.2rem', color: 'white' }}>
                                  <FaProjectDiagram />
                                </div>
                                <div>
                                  <h3 className="mb-0 fw-bold" style={{ fontSize: '1.2rem', color: 'white' }}>{projectData.projectName}</h3>
                                </div>
                              </div>
                              {(data?.role === 'project_manager' || data?.role === 'team_lead') && (
                                <div className="d-flex gap-1">
                                  <button
                                    className="btn btn-light btn-sm rounded-pill px-2"
                                    style={{ fontSize: '0.8rem', padding: '0.25rem 0.5rem' }}
                                    onClick={() => openBacklogModal(project)}
                                  >
                                    <FaPlus className="me-1" />
                                    Add Backlog
                                  </button>
                                </div>
                              )}
                            </div>
                          </div>
                          {projectData.backlogs.length === 0 ? (
                            <div className="text-center text-muted mb-3">
                              <p className="mb-2" style={{ fontSize: '0.9rem' }}>No backlog items for this project.</p>
                              {(data?.role === 'project_manager' || data?.role === 'team_lead') && (
                                <button
                                  className="btn btn-outline-primary btn-sm rounded-pill px-2"
                                  style={{ 
                                    fontWeight: '600',
                                    fontSize: '0.85rem',
                                    boxShadow: '0 2px 8px rgba(0,0,0,0.1)'
                                  }}
                                  onClick={() => openBacklogModal(projectData.project)}
                                >
                                  <FaPlus className="me-1" />
                                  Add First Backlog Item
                                </button>
                              )}
                            </div>
                          ) : (
                            <div className="row g-3">
                              {projectData.backlogs.map((backlog, backlogIdx) => (
                                <div key={backlog.backlog_id || backlogIdx} className="col-12 col-sm-6 col-lg-4 col-xl-3">
                                  <div className="card h-100 shadow-sm border-0 backlog-card themed-card"
                                    style={{
                                      transition: 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1), box-shadow 0.3s cubic-bezier(0.4, 0, 0.2, 1), background 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                                      cursor: 'pointer',
                                      willChange: 'transform, box-shadow, background'
                                    }}
                                    onMouseEnter={(e) => {
                                      e.currentTarget.style.transform = 'translateY(-4px)';
                                      e.currentTarget.style.boxShadow = theme === 'dark' 
                                        ? '0 15px 30px rgba(0,0,0,0.4)' 
                                        : '0 15px 30px rgba(0,0,0,0.15)';
                                      e.currentTarget.style.background = theme === 'dark' 
                                        ? 'linear-gradient(145deg, #343a40 0%, #3d4248 100%)' 
                                        : 'linear-gradient(145deg, #f8f9fa 0%, #e9ecef 100%)';
                                    }}
                                    onMouseLeave={(e) => {
                                      e.currentTarget.style.transform = 'translateY(0)';
                                      e.currentTarget.style.boxShadow = theme === 'dark' 
                                        ? '0 4px 20px rgba(0,0,0,0.3)' 
                                        : '0 4px 20px rgba(0,0,0,0.08)';
                                      e.currentTarget.style.background = theme === 'dark' 
                                        ? 'linear-gradient(145deg, #2b2f36 0%, #343a40 100%)' 
                                        : 'linear-gradient(145deg, #ffffff 0%, #f8f9fa 100%)';
                                    }}
                                  >
                                    <div className="card-body p-3">
                                      <div className="d-flex justify-content-between align-items-start mb-2">
                                        <h5 className="card-title mb-0 fw-bold" style={{ fontSize: '1.2rem' }}>
                                          {backlog.title}
                                        </h5>
                                                                                  <div className="text-end" style={{ marginRight: '8px' }}>
                                            <small className="text-muted d-block" style={{ fontSize: '0.75rem', marginBottom: '2px' }}>Status:</small>
                                            <span className={`badge rounded-pill px-2 py-1 ${
                                              backlog.sprint_id 
                                                ? 'bg-success' 
                                                : theme === 'dark' ? 'bg-secondary' : 'bg-light text-dark'
                                            }`} style={{
                                              fontSize: '0.85rem',
                                              fontWeight: '1000',
                                              padding: '0.25rem 0.5rem'
                                            }}>
                                              {backlog.sprint_id ? '🏃 In Sprint' : '📝 Available'}
                                            </span>
                                          </div>
                                      </div>
                                      
                                      <div className="mb-3">
                                        <small className="text-muted d-block" style={{ fontSize: '0.75rem', fontWeight: '500', marginBottom: '4px' }}>Description</small>
                                        <p className="card-text text-muted mb-0" style={{
                                          fontSize: '0.95rem',
                                          lineHeight: '1.5',
                                          opacity: '0.8'
                                        }}>
                                          {backlog.description || 'No description provided'}
                                        </p>
                                      </div>
                                      
                                      <div className="mt-auto">
                                        {(data?.role === 'project_manager' || data?.role === 'team_lead') && (
                                          <div className="mb-2">
                                            <div className="d-flex justify-content-between align-items-center">
                                              <div className="btn-group" role="group">
                                                {!backlog.sprint_id && (
                                                  <button
                                                    className="btn btn-primary btn-sm rounded-pill me-1"
                                                    onClick={() => {
                                                      setSelectedProjectId(projectId);
                                                      openSprintModal(backlog);
                                                    }}
                                                    style={{
                                                      fontSize: '0.85rem',
                                                      fontWeight: '500'
                                                    }}
                                                  >
                                                    <FaPlus className="me-1" />
                                                    Add to Sprint
                                                  </button>
                                                )}
                                                <button
                                                  className="btn btn-danger btn-sm rounded-pill"
                                                  onClick={() => handleDeleteBacklog(backlog.backlog_id || backlog.id)}
                                                  style={{
                                                    fontSize: '0.85rem',
                                                    fontWeight: '500'
                                                  }}
                                                >
                                                  <FaTrash className="me-1" />
                                                  Delete
                                                </button>
                                              </div>
                                            </div>
                                          </div>
                                        )}
                                        
                                        {backlog.created_at && (
                                          <small className="text-muted d-block" style={{ fontSize: '0.9rem' }}>
                                            📅 Created: {new Date(backlog.created_at).toLocaleDateString('en-US', {
                                              year: 'numeric',
                                              month: 'short',
                                              day: 'numeric'
                                            })}
                                          </small>
                                        )}
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              ))}
                            </div>
                          )}
                        </div>
                      );
                    })}
                  </div>
                )}
              </div>
            </div>
          </div>
        </div>

        {/* Enhanced Inline Styles */}
        <style>{`
          :root {
            --input-bg: #fff;
            --input-color: #212529;
            --input-border: #ced4da;
            --input-focus: #86b7fe;
            --placeholder-color: #6c757d;
            --card-bg: #fff;
            --card-text: #212529;
          }

          body.dark {
            --input-bg: #2c3039;
            --input-color: #f1f1f1;
            --input-border: #495057;
            --input-focus: #5c9eff;
            --placeholder-color: #adb5bd;
            --card-bg: #2b2f36;
            --card-text: #f1f1f1;
          }

          .themed-card {
            background-color: var(--card-bg) !important;
            color: var(--card-text) !important;
          }

          .themed-card .list-group-item {
            background-color: var(--card-bg) !important;
            color: var(--card-text) !important;
            border-color: var(--input-border) !important;
          }

          .form-control,
          .form-select {
            background-color: var(--input-bg);
            color: var(--input-color);
            border: 1px solid var(--input-border);
            border-radius: 12px;
            padding: 12px 16px;
            font-size: 0.95rem;
            transition: all 0.3s ease;
          }

          .form-control::placeholder {
            color: var(--placeholder-color);
          }

          .form-control:focus,
          .form-select:focus {
            border-color: var(--input-focus);
            box-shadow: 0 0 0 0.2rem rgba(13, 110, 253, 0.25);
            transform: translateY(-2px);
          }

          .btn {
            border-radius: 12px;
            font-weight: 500;
            transition: all 0.3s ease;
            border: none;
          }

          .btn:hover {
            transform: translateY(-2px);
            box-shadow: 0 8px 25px rgba(0,0,0,0.15);
          }

          .btn-primary {
            background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%);
            box-shadow: 0 4px 15px rgba(59, 130, 246, 0.3);
          }

          .btn-danger {
            background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%);
            box-shadow: 0 4px 15px rgba(239, 68, 68, 0.3);
          }

          .btn-success {
            background: linear-gradient(135deg, #10b981 0%, #059669 100%);
            box-shadow: 0 4px 15px rgba(16, 185, 129, 0.3);
          }

          .backlog-card {
            backdrop-filter: blur(10px);
            border: 1px solid rgba(255, 255, 255, 0.1);
          }

          .project-header {
            backdrop-filter: blur(10px);
            border: 1px solid rgba(255, 255, 255, 0.2);
          }

          .modal-content {
            border-radius: 20px;
            border: none;
            backdrop-filter: blur(20px);
          }

          .modal-header {
            border-bottom: 1px solid rgba(255, 255, 255, 0.1);
            border-radius: 20px 20px 0 0;
          }

          .modal-footer {
            border-top: 1px solid rgba(255, 255, 255, 0.1);
            border-radius: 0 0 20px 20px;
          }

          .spinner-border {
            border-width: 3px;
          }

          html, body {
            max-width: 100%;
            overflow-x: hidden;
          }

          .row {
            margin-right: 0;
            margin-left: 0;
          }

          /* Custom scrollbar */
          ::-webkit-scrollbar {
            width: 8px;
          }

          ::-webkit-scrollbar-track {
            background: transparent;
          }

          ::-webkit-scrollbar-thumb {
            background: rgba(0,0,0,0.2);
            border-radius: 10px;
          }

          body.dark ::-webkit-scrollbar-thumb {
            background: rgba(255,255,255,0.2);
          }

          /* Animation for cards */
          @keyframes fadeInUp {
            from {
              opacity: 0;
              transform: translateY(30px);
            }
            to {
              opacity: 1;
              transform: translateY(0);
            }
          }

          .backlog-card {
            animation: fadeInUp 0.6s ease forwards;
          }

          .backlog-card:nth-child(1) { animation-delay: 0.1s; }
          .backlog-card:nth-child(2) { animation-delay: 0.2s; }
          .backlog-card:nth-child(3) { animation-delay: 0.3s; }
          .backlog-card:nth-child(4) { animation-delay: 0.4s; }
          .backlog-card:nth-child(5) { animation-delay: 0.5s; }
          .backlog-card:nth-child(6) { animation-delay: 0.6s; }
        `}</style>
        {/* End Enhanced Inline Styles */}
      </div> {/* Close .container-fluid */}
            {/* Backlog Modal */}
            {(data?.role === 'project_manager' || data?.role === 'team_lead') && (
  <>
    <Modal show={showBacklogModal} onHide={closeBacklogModal} size="lg" centered>
      <Modal.Header closeButton>
        <Modal.Title style={{ fontSize: '1.1rem' }}>Project Backlog</Modal.Title>
      </Modal.Header>
      <Modal.Body style={{ padding: '1rem' }}>
        <h5 style={{ fontSize: '1rem' }}>Add Backlog Item</h5>
        <div className="row g-2 mb-3">
          <div className="col-md-4">
            <input type="text" className="form-control" placeholder="Title" value={newBacklog.title} onChange={e => setNewBacklog(nb => ({ ...nb, title: e.target.value }))} required />
          </div>
          <div className="col-md-6">
            <input type="text" className="form-control" placeholder="Description" value={newBacklog.description} onChange={e => setNewBacklog(nb => ({ ...nb, description: e.target.value }))} />
          </div>
          <div className="col-md-2">
            <button className="btn btn-success w-100" onClick={handleAddBacklog} style={{ fontSize: '1rem', padding: '0.5rem', height: '48px' }}>Add</button>
          </div>
        </div>
      </Modal.Body>
      <Modal.Footer style={{ padding: '0.75rem 1rem' }}>
        <Button variant="secondary" onClick={closeBacklogModal} size="sm" style={{ fontSize: '0.8rem' }}>Close</Button>
      </Modal.Footer>
    </Modal>
    {/* Add to Sprint Modal */}
    <Modal show={showSprintModal} onHide={closeSprintModal} centered>
      <Modal.Header closeButton>
        <Modal.Title style={{ fontSize: '1.1rem' }}>Add to Sprint</Modal.Title>
      </Modal.Header>
      <Modal.Body style={{ padding: '1rem' }}>
        <div className="mb-2">
          <label className="form-label" style={{ fontSize: '0.8rem', fontWeight: '600' }}>Sprint Description</label>
          <textarea 
            className="form-control" 
            value={sprintDesc} 
            onChange={e => setSprintDesc(e.target.value)} 
            style={{ fontSize: '0.8rem', padding: '0.5rem' }}
          />
        </div>
        <div className="mb-2">
          <label className="form-label" style={{ fontSize: '0.8rem', fontWeight: '600' }}>Start Date</label>
          <input 
            type="date" 
            className="form-control" 
            value={new Date().toISOString().slice(0,10)} 
            readOnly 
            style={{ fontSize: '0.8rem', padding: '0.5rem' }}
          />
        </div>
        <div className="mb-2">
          <label className="form-label" style={{ fontSize: '0.8rem', fontWeight: '600' }}>End Date <span className="text-danger">*</span></label>
          <input 
            type="date" 
            className="form-control" 
            value={sprintEndDate} 
            min={new Date().toISOString().slice(0,10)} 
            onChange={e => setSprintEndDate(e.target.value)} 
            required 
            style={{ fontSize: '0.8rem', padding: '0.5rem' }}
          />
          {sprintEndDateError && <div className="text-danger mt-1" style={{ fontSize: '0.75rem' }}>{sprintEndDateError}</div>}
        </div>
      </Modal.Body>
      <Modal.Footer style={{ padding: '0.75rem 1rem' }}>
        <Button variant="secondary" onClick={closeSprintModal} size="sm" style={{ fontSize: '0.8rem' }}>Cancel</Button>
        <Button variant="primary" onClick={handleAddToSprint} size="sm" style={{ fontSize: '0.8rem' }}>Add to Sprint</Button>
      </Modal.Footer>
    </Modal>
  </>
)}

{/* Project Selection Modal */}
<Modal show={showProjectSelectionModal} onHide={closeProjectSelectionModal} centered>
  <Modal.Header closeButton>
    <Modal.Title style={{ fontSize: '1.1rem' }}>Select Project for Backlog Item</Modal.Title>
  </Modal.Header>
  <Modal.Body style={{ padding: '1rem' }}>
    <p className="text-muted mb-3" style={{ fontSize: '0.8rem' }}>Choose which project you want to add the backlog item to:</p>
    <div className="row g-2">
      {projects.map((project) => (
        <div key={project.project_id || project.pid} className="col-12">
          <div 
            className="card project-selection-card"
            style={{
              cursor: 'pointer',
              transition: 'all 0.3s ease',
              border: '2px solid transparent',
              background: theme === 'dark' 
                ? 'linear-gradient(145deg, #2b2f36 0%, #343a40 100%)' 
                : 'linear-gradient(145deg, #ffffff 0%, #f8f9fa 100%)'
            }}
            onMouseEnter={(e) => {
              e.currentTarget.style.transform = 'translateY(-2px)';
              e.currentTarget.style.borderColor = '#3b82f6';
              e.currentTarget.style.boxShadow = '0 8px 25px rgba(59, 130, 246, 0.15)';
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.transform = 'translateY(0)';
              e.currentTarget.style.borderColor = 'transparent';
              e.currentTarget.style.boxShadow = 'none';
            }}
            onClick={() => handleProjectSelection(project)}
          >
            <div className="card-body p-3">
              <div className="d-flex align-items-center">
                <div className="me-2">
                  <div 
                    className="rounded-circle d-flex align-items-center justify-content-center"
                    style={{
                      width: '32px',
                      height: '32px',
                      background: 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)',
                      color: 'white',
                      fontSize: '1rem'
                    }}
                  >
                    <FaProjectDiagram />
                  </div>
                </div>
                <div>
                  <h6 className="mb-1 fw-bold" style={{ fontSize: '0.9rem' }}>{project.name || project.pname}</h6>
                  <small className="text-muted" style={{ fontSize: '0.75rem' }}>Click to select this project</small>
                </div>
              </div>
            </div>
          </div>
        </div>
      ))}
    </div>
  </Modal.Body>
  <Modal.Footer style={{ padding: '0.75rem 1rem' }}>
    <Button variant="secondary" onClick={closeProjectSelectionModal} size="sm" style={{ fontSize: '0.8rem' }}>Cancel</Button>
  </Modal.Footer>
</Modal>
    </div>
  );
}

export default BacklogPage;